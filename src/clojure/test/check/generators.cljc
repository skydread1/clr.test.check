...
#?(:cljr                                                                                        ;;; Changed :clj to :cljr
   (defn ^:private two-pow
     [exp]
     (bigint (.LeftShift (biginteger 1) exp))))                                                 ;;; .shiftLeft
#?(:cljr (def ^:private dec-2-32 0xFFFFFFFF))                                                   ;;; Changed :clj to :cljr
#?(:cljr (def ^:private just-2-32 0x100000000))                                                 ;;; Changed :clj to :cljr
  ...
  between -(2^(max-bit-length)) and 2^(max-bit-length)."
     ...
     (fmap
      (fn [xs]
        ...
        (loop [multiple  (or false 1)                                                                ;;; (Long. 1)  -- added the or-false wrapper
               bits-left max-bit-length
               xs        xs
               res       (or false 0)]                                                               ;;; (Long. 0) -- added the or-false wrapper
          (cond (<= 32 bits-left)
...
(defn ^:private resize-symbolish-generator
  ...
  (scale #(long (Math/Pow % 0.60)) g))                                         ;;; Math/pow
...
(one-of [int #?(:clj size-bounded-bigint :cljs large-integer :cljr size-bounded-bigint ) double-gen char-gen              ;;; Added :cljr clause
...
(defn container-type
  ...
  (one-of [(vector inner-type)
           (list inner-type)
           (set #?(:default inner-type                                         ;;; changed :clj to :default
                   :cljs (such-that hashable? inner-type)))
           ;; scaling this by half since it naturally generates twice
           ;; as many elements
           (scale #(quot % 2)
                  (map #?(:default inner-type                                 ;;; changed :clj to :default
                          :cljs (such-that hashable? inner-type))
                       inner-type))]))
